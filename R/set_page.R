...
set_page_orientation <- function(
  ...
  page[switch_pg, ][[orientation_col]] <- as_orientation(
  ...
  if (any(!(orientation == page[[orientation_col]]))) {
    cli_warn(
      c(
        "{.arg orientation} can't be set to {.val {orientation}}
...
set_page_asp <- function(page, flipped = FALSE, cols = c("width", "height")) {
  page[[get_asp_col()]] <- as_asp(
    page = page,
    flipped = flipped,
    cols = cols
  )
